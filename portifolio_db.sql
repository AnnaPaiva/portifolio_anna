...
--
-- Host: 127.0.0.1
-- Tempo de geração: 24/03/2026 às 21:12
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_projects_by_category`  AS SELECT `projects`.`id` AS `id`, `projects`.`title` AS `title`, `projects`.`description` AS `description`, `projects`.`category_id` AS `category_id`, `categories`.`name` AS `category_name` FROM (`projects` left join `categories` on(`projects`.`category_id` = `categories`.`id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_projects_summary`  AS SELECT `projects`.`id` AS `id`, `projects`.`title` AS `title`, `projects`.`description` AS `description`, `categories`.`name` AS `category_name`, `projects`.`creation_date` AS `creation_date` FROM (`projects` left join `categories` on(`projects`.`category_id` = `categories`.`id`)) ;
